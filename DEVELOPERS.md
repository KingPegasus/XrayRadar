...
Local setup, deployment, testing, security tooling, and API reference for **XrayRadar**. For a short project overview and quick start, see [README.md](README.md).

Copy [`.env.example`](.env.example) to `.env` and adjust values (the real `.env` file is gitignored).
...
```bash
docker build -t xrayradar:local .
```
...
```bash
# Default Compose network: "${COMPOSE_PROJECT_NAME:-<repo-folder-name>}_default"
NETWORK="${COMPOSE_PROJECT_NAME:-$(basename "$PWD")}_default"
docker run --rm \
  --name xrayradar-local \
  --network "$NETWORK" \
  -p 8001:8000 \
  --env-file .env \
...
Run these commands from the repository root (same place you ran `docker compose up -d`). If you set `COMPOSE_PROJECT_NAME` in the environment or in a Compose `.env` file, use that name: the network is `${COMPOSE_PROJECT_NAME}_default`. You can confirm with `docker compose config --format json` and inspect `networks.default.name`, or `docker network ls`.
...
- `RESEND_FROM_EMAIL` — From address for all emails (e.g. `alerts@example.com`); must be a verified sending domain in Resend.
- `XRAYRADAR_BASE_URL` — Base URL for links and the logo in emails (verification, password reset, alert links). Defaults to `http://localhost:8001` if unset. **In production set this to your public app URL** so the logo and links work when recipients open the email; a localhost URL will not load in their client.
...
Run the test suite with coverage locally (from this repository’s root directory):
...
**Note**: Run backend tests from the repository root (not `xrayradar-web`).
